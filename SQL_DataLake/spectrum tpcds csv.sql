--q1.sql--
 ...
 WITH customer_total_return AS
   (SELECT sr_customer_sk AS ctr_customer_sk, sr_store_sk AS ctr_store_sk,
    ...
    FROM csv.store_returns, csv.date_dim
    WHERE sr_returned_date_sk = d_date_sk AND d_year = 2000
    GROUP BY sr_customer_sk, sr_store_sk)
 SELECT c_customer_id
   FROM customer_total_return ctr1, csv.store, csv.customer
...
SELECT state, cnt FROM (
 ...
 FROM
    csv.customer_address a, csv.customer c, csv.store_sales s, csv.date_dim d, csv.item i
 WHERE a.ca_address_sk = c.c_current_addr_sk
  AND c.c_customer_sk = s.ss_customer_sk
  AND s.ss_sold_date_sk = d.d_date_sk
  AND s.ss_item_sk = i.i_item_sk
  AND d.d_month_seq =
       (SELECT distinct (d_month_seq) FROM csv.date_dim 
        WHERE d_year = 2001 AND d_moy = 1)
  AND i.i_current_price > 1.2 *
             (SELECT avg(j.i_current_price) FROM csv.item j
...
--q30.sql--
 ...
 from csv.web_returns, csv.date_dim, csv.customer_address
 ...
 from customer_total_return ctr1, csv.customer_address, csv.customer
...
--q47.sql--
 ...
 from csv.item, csv.store_sales, csv.date_dim, csv.store
     ...
     (select i_category, i_class, i_brand, i_product_name, d_year, d_qoy, d_moy, s_store_id,
      ...
      from
         (select i_category, i_class, i_brand, i_product_name, d_year, d_qoy, d_moy,
          ...
          from csv.store_sales, csv.date_dim, csv.store, csv.item
...
--q88.sql--
 ...
 from
   (select count(*) h8_30_to_9
    from csv.store_sales, csv.household_demographics , csv.time_dim, csv.store
